...
layui.define('layer', function(exports){
    ...
    var submit = function(){
        var button = $(this), verify = form.config.verify, stop = null
            ...
            ,verifyElem = elem.find('*[lay-verify]:not([disabled])') //获取需要校验的元素
            ,formElem = button.parents('form')[0] //获取当前所在的form元素，如果存在的话
            ,fieldElem = elem.find('input:not([disabled]),select:not([disabled]),textarea:not([disabled])') //获取所有表单域
            ,filter = button.attr('lay-filter'); //获取过滤器

        //开始校验
        layui.each(verifyElem, function(_, item){
            var othis = $(this), ver = othis.attr('lay-verify'), msg = othis.attr('lay-msg'), tips = '';
