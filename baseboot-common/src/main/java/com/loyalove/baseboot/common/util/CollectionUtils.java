...
public abstract class CollectionUtils {
	...
	/**
	 * 返回由指定映射支持的同步（线程安全的）{@link MultiValueMap} 。
	 * 为了保证按顺序访问，必须通过返回的映射完成对底层映射的<strong>所有</strong>访问。
	 * 
	...
	static class SynchronizedMultiValueMap<K, V> implements MultiValueMap<K, V> {
		...
		private transient Set<Entry<K, V>> singleValueEntrySet;
		...
		public Set<Entry<K, List<V>>> entrySet() {
			synchronized (this.mutex) {
				if (this.entrySet == null) {
					this.entrySet = new SynchronizedSet<Entry<K, List<V>>>(this.multiValueMap.entrySet(), this.mutex);
				}
	 ...
	 * 
	 */
	private static class EmptyMultiValueMap extends AbstractMap<Object, List<Object>> implements
	...
	/**
	 * 返回指定映射的不可修改视图。此方法允许使用模块，为用户提供对内部映射的“只读”访问。在返回的映射上执行的查询操作将“读完”指定的映射。
	 * 试图修改返回的映射，不管是直接修改还是通过其 collection 视图进行修改，都将导致抛出
	 * UnsupportedOperationException。
	 * <p>
	 * 如果指定映射是可序列化的，则返回的映射也将是可序列化的。
	 * 
	...
	static class UnmodifiableMultiValueMap<K, V> implements MultiValueMap<K, V>, Serializable {
		...
		public Set<Entry<K, V>> singleValueEntrySet() {
